...
export function FeedbackModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 font-montserrat animate-fade-in">
      <div className="bg-[#141414] w-full max-w-lg rounded-3xl border border-[#e3a638]/40 shadow-2xl p-6 text-white relative">
...
export default function FeedbackWidget() {
  ...
  return (
    <>
      {/* Floating Button ONLY on Desktop (hidden on mobile md:flex) */}
      <button
        onClick={() => setOpen(true)}
        className="hidden md:flex fixed bottom-6 left-6 z-40 px-4 py-3 bg-stone-900/90 text-[#e3a638] border border-[#e3a638]/40 hover:bg-stone-900 rounded-full shadow-2xl backdrop-blur-md items-center gap-2 text-xs font-bold uppercase tracking-wider transition-all hover:scale-105 cursor-pointer font-montserrat"
      >
