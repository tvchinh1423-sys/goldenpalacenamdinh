...
function MenuContent() {
  ...
  return (
    <div className="bg-[#fcf9f2] text-gray-900 font-montserrat min-h-screen flex flex-col pt-20 sm:pt-24 pb-28 w-full overflow-x-hidden">
      
      {/* Banner Title */}
      <section className="max-w-7xl mx-auto px-3.5 sm:px-6 mb-6 sm:mb-8 text-center">
        <span className="text-[#a66a3a] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-[10px] sm:text-xs font-bold inline-block">
          Ẩm Thực Đỉnh Cao Golden Palace
        </span>
        <h1 className="text-2xl sm:text-5xl font-playfair font-bold text-gray-900 mt-1 sm:mt-2 mb-2 sm:mb-3">
          Thực Đơn Tiệc & Bảng Giá Chính Thức
        </h1>
        <p className="text-gray-600 font-light text-xs sm:text-sm max-w-3xl mx-auto leading-relaxed">
      ...
      <section className="max-w-7xl mx-auto px-3.5 sm:px-6 mb-6 sm:mb-10">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 bg-white p-2 sm:p-3 rounded-2xl shadow-xl border border-[#e3a638]/30">
          {MENU_CATEGORIES.map(cat => {
            ...
            return (
              <button
                ...
                className={`p-2.5 sm:p-3.5 rounded-xl flex flex-col items-center justify-center text-center transition-all duration-300 cursor-pointer ${
                  isActive 
              ...
              >
                <span className={`material-symbols-outlined text-xl sm:text-2xl mb-1 ${isActive ? 'text-[#e3a638]' : 'text-[#a66a3a]'}`}>
                  {cat.icon}
                </span>
                <span className="font-playfair font-semibold text-[11px] sm:text-xs leading-tight mb-1">{cat.label}</span>
                <span className={`text-[9px] sm:text-[10px] font-light ${isActive ? 'text-amber-200/80' : 'text-gray-400'}`}>{cat.count}</span>
              </button>
            );
          })}
        </div>
      ...
      {activeTab === 'SET_TIEC' && (
        <section className="max-w-7xl mx-auto px-3.5 sm:px-6 space-y-6 sm:space-y-8">
          ...
          <div className="flex items-center justify-between bg-white px-2.5 sm:px-6 py-2.5 sm:py-3 rounded-2xl border border-[#e3a638]/30 shadow-md gap-1.5 sm:gap-3 w-full max-w-2xl mx-auto">
            <button 
              onClick={() => setSetMenuIndex(prev => Math.max(0, prev - 1))}
              disabled={setMenuIndex === 0}
              className="flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-xs font-bold uppercase tracking-wider px-2.5 sm:px-4 py-2 rounded-xl bg-amber-50 text-[#a66a3a] border border-[#e3a638]/40 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#e3a638] hover:text-white transition-all cursor-pointer whitespace-nowrap shrink-0"
            >
            ...
            {/* Pagination Badges */}
            <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto min-w-0 flex-1 justify-center py-1 no-scrollbar">
              {SET_MENUS_18.map((m, idx) => (
                <button
                  key={idx}
                  onClick={() => setSetMenuIndex(idx)}
                  className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full text-[10px] sm:text-xs font-bold transition-all cursor-pointer flex items-center justify-center shrink-0 ${
                    setMenuIndex === idx 
          ...
          <div className="max-w-2xl mx-auto w-full">
            {(() => {
              ...
              return (
                <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-[#e3a638] shadow-2xl overflow-hidden flex flex-col justify-between transition-all w-full">
                  
                  {/* Card Header */}
                  <div className="bg-gradient-to-r from-gray-900 via-amber-950 to-gray-900 text-white p-4 sm:p-6 border-b border-[#e3a638]/40">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
                      <h3 className="text-xl sm:text-2xl font-playfair font-bold text-[#e3a638]">{menu.title}</h3>
                      <span className="self-start sm:self-auto bg-[#e3a638]/20 border border-[#e3a638] text-[#e3a638] text-[9px] sm:text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-widest shrink-0">
                        {setMenuIndex + 1} / 18 SET MENUS
                      </span>
                    </div>
                    <p className="text-base sm:text-lg font-bold text-white font-playfair">{menu.price}</p>
                  </div>

                  {/* Dishes List */}
                  <div className="p-4 sm:p-6 space-y-2 sm:space-y-3">
                    {menu.dishes.map((dish, dIdx) => (
                      <div key={dIdx} className="flex justify-between items-center text-xs sm:text-sm py-1 sm:py-1.5 border-b border-gray-100 last:border-0 gap-2">
                        <span className="font-medium text-gray-800 leading-snug">{dIdx + 1}. {dish.name}</span>
                        <span className="text-[9px] sm:text-[10px] font-semibold text-[#a66a3a] bg-amber-50 px-2 py-0.5 rounded-full shrink-0 ml-1 whitespace-nowrap">{dish.type}</span>
                      </div>
                    ))}
                  </div>

                  {/* Card Action Footer */}
                  <div className="p-4 sm:p-5 bg-[#fcf9f2] border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
                    <span className="text-xs text-gray-500 font-light text-center sm:text-left">Mâm 10 khách chuẩn chỉnh</span>
                    <button 
                      onClick={() => setIsModalOpen(true)}
                      className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-[#e3a638] to-[#a66a3a] text-white hover:opacity-90 text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md cursor-pointer whitespace-nowrap text-center"
                    >
          ...
          <div className="pt-6 sm:pt-8">
            <h3 className="text-center font-playfair font-bold text-lg sm:text-xl text-gray-900 mb-4 sm:mb-6">Tất Cả 18 Set Menu (Bấm để xem chi tiết)</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
              {SET_MENUS_18.map((m, idx) => (
                ...
                >
                  <span className="block text-[9px] sm:text-[10px] uppercase text-[#a66a3a] font-bold">SET {idx + 1}</span>
                  <span className="block text-[11px] sm:text-xs font-bold mt-0.5 whitespace-nowrap">{m.price.split(' ')[0]}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'CHUYEN_MON' && (
        <section className="max-w-7xl mx-auto px-3.5 sm:px-6 space-y-8">
      ...
      {activeTab === 'TRE_EM' && (
        <section className="max-w-7xl mx-auto px-3.5 sm:px-6 space-y-8">
          <div className="bg-gradient-to-r from-amber-900 via-amber-950 to-gray-900 text-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#e3a638]/40 mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-playfair font-bold text-white mb-2">Ưu Đãi Đặc Biệt Cho Học Sinh & Thầy Cô</h2>
            ...
            <button 
              onClick={() => setIsModalOpen(true)}
              className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-[#e3a638] to-[#a66a3a] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-xl hover:scale-105 transition-transform cursor-pointer whitespace-nowrap text-center"
            >
              Đặt Tiệc Hè Rực RỠ ➔
            </button>
          </div>
      ...
      {activeTab === 'ALACARTE' && (
        <section className="max-w-7xl mx-auto px-3.5 sm:px-6 space-y-8">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-6 shadow-md flex items-start gap-3 sm:gap-4 text-amber-900">
            <span className="material-symbols-outlined text-amber-700 text-2xl sm:text-3xl shrink-0 mt-0.5">info</span>
            <div>
              <h3 className="font-playfair font-bold text-base sm:text-lg text-amber-950 mb-1">Lưu ý về Thực Đơn Chọn Món Tươi Sống:</h3>
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            <button
              ...
              className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-[11px] sm:text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
          ...
          <div className="space-y-6 sm:space-y-10">
            {ALACARTE_3_SECTIONS
              ...
              .map((sec, secIdx) => (
                <div key={secIdx} className="bg-white rounded-2xl sm:rounded-3xl border border-gray-200 p-4 sm:p-8 shadow-xl">
                  
                  {/* SECTION HEADER */}
                  <div className="flex items-center gap-3 pb-3 sm:pb-4 mb-4 sm:mb-6 border-b-2 border-[#e3a638]/40">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-amber-100 text-[#a66a3a] flex items-center justify-center shadow-inner shrink-0">
                      <span className="material-symbols-outlined text-xl sm:text-2xl">{sec.icon}</span>
                    </div>
                    <div>
                      <h2 className="text-xl sm:text-2xl font-playfair font-bold text-gray-900">{sec.sectionTitle}</h2>
                      <p className="text-[11px] sm:text-xs text-gray-500 font-light mt-0.5">{sec.sectionDesc}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    {sec.groups.map((grp, grpIdx) => (
                      <div key={grpIdx} className="bg-[#fcf9f2] rounded-2xl border border-gray-200 p-4 sm:p-5 flex flex-col justify-between">
                        <div>
                          <h3 className="text-sm sm:text-base font-playfair font-bold text-gray-900 pb-2 border-b border-[#e3a638]/30 flex items-center gap-2">
                            <span className="text-[#e3a638] text-xs sm:text-sm">✨</span> {grp.subTitle}
                          </h3>
                          
                          <div className="mt-3 sm:mt-4 space-y-2 max-h-[400px] overflow-y-auto pr-1">
                            {grp.dishes.map((dish, dIdx) => {
                              ...
                              return (
                                <div 
                                  ...
                                  className={`p-2 sm:p-2.5 rounded-xl border text-xs flex items-center justify-between cursor-pointer transition-all ${
                                    isChecked 
                                ...
                                >
                                  <div className="flex items-center gap-2 pr-2 leading-snug">
                                    <input 
                                      type="checkbox" 
                                      checked={isChecked}
                                      onChange={() => {}}
                                      className="accent-[#e3a638] w-4 h-4 cursor-pointer shrink-0"
                                    />
                                    <span>{dish}</span>
                                  </div>
                                  <span className={`text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full shrink-0 whitespace-nowrap ${isChecked ? 'bg-[#e3a638] text-white font-bold' : 'bg-gray-100 text-gray-500'}`}>
      ...
      {activeTab === 'DO_UONG' && (
        <section className="max-w-7xl mx-auto px-3.5 sm:px-6 space-y-6 sm:space-y-10">
          <div className="text-center mb-4 sm:mb-6">
            <h2 className="text-xl sm:text-2xl font-playfair font-bold text-gray-900">Bảng Giá Đồ Uống & Phí Mang Đồ Vào Nhà Hàng</h2>
