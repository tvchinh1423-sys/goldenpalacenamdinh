  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-headline-sm text-on-surface">Quản lý Dịch vụ bổ sung</h2>
        ...
        <table className="w-full text-left font-body-md">
          <thead className="bg-surface-container-lowest text-on-surface-variant border-b border-outline-variant/30">
            <tr>
              ...
              <th className="px-6 py-4 font-label-md font-semibold">Tên dịch vụ</th>
          ...
          <tbody className="divide-y divide-outline-variant/20">
              ...
              return (
                <tr key={a.id} className="hover:bg-surface-variant/30 transition-colors">
                  <td className="px-6 py-4">
                    <img src={thumbnail} alt={a.name} className="w-12 h-12 rounded object-cover border border-outline-variant/30" />
                  </td>
                  <td className="px-6 py-4 font-bold text-on-surface">{a.name}</td>
                  <td className="px-6 py-4 text-on-surface-variant">
                    {a.pricings.length > 0 
                      ? `${a.pricings.length} mức giá (Từ ${formatCurrency(a.pricings[0].price)})` 
                      : 'Chưa cài đặt giá'}
                  </td>
                  <td className="px-6 py-4">
                    <form action={toggleStatus}>
                      ...
                      <button type="submit" className={`px-3 py-1 text-xs rounded-full font-label-md flex items-center gap-1 border ${
                        ...
                        <span className="material-symbols-outlined text-[14px]">
                          {a.status === 'PUBLISHED' ? 'public' : 'visibility_off'}
                        </span>
            ...
            {addons.length === 0 && (
              <tr>
                <td colSpan="5" className="px-6 py-10 text-center text-on-surface-variant">Không có dữ liệu</td>
              </tr>
            )}
          </tbody>
        </table>
